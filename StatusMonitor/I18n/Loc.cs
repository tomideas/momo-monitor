...
public sealed class Loc : INotifyPropertyChanged
{
    ...
    private static readonly Dictionary<string, (string en, string zh)> Map = new()
    {
        ...
        ["power_missing_gpu"] = ("A card reports no power and is not in the total",
                                 "有顯示卡不回報功耗，未計入總數"),
        ["power_now_wall"] = ("Estimated power at the wall", "估算牆插功耗"),
        ["wall_power_title"] = ("Whole-machine power at the wall", "整機牆插功率"),
        ["wall_power"] = ("Count the power supply's losses", "計入電源供應器損耗"),
        ["wall_power_hint"] = (
            "The figure above is what the parts draw. A power supply burns some of what it converts, and the less it is asked for the worse that gets — a desktop idling at 30 W sits far below the load where its rating is defined. Switch this on to show what the wall meter would see instead. It needs both figures below; they are printed on the supply's own label.",
            "上面的數字是零件的耗電。電源供應器在轉換時會燒掉一部分，而且要得越少、燒得越兇 — 一台閒置在 30 W 的桌機，遠低於它額定效率被定義的負載區間。打開這個開關，顯示的就改成牆插功率計會看到的數字。需要下面兩項，兩項都印在電源供應器自己的標籤上。"),
        ["psu_rated"] = ("Supply rating", "額定瓦數"),
        ["psu_class"] = ("80 PLUS rating", "80 PLUS 等級"),
        ["psu_none"] = ("80 PLUS / unrated", "80 PLUS／無認證"),
        ["invalid_psu"] = ("Enter a rating between 100 and 2000 W", "請輸入 100 至 2000 W 之間的額定瓦數"),
        ["gpu_tdp"] = ("Board power of unrecognised cards", "未識別顯示卡的額定功耗"),
        ["gpu_tdp_hint"] = (
            "These cards report no power draw, and this build does not know their rating. Enter the board power printed in their specifications and they will be estimated like the rest; leave it blank and they stay out of the total.",
            "這些顯示卡不回報功耗，本程式也不認得它們的型號。填入規格表上的額定功耗即可比照其他硬體估算；留空則維持不計入總數。"),
